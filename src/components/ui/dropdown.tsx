...
export function DropdownItem({
  ...
  const classes = clsx(
    className,
    // Base styles
    'group cursor-pointer rounded-md px-2.5 py-1.5 focus:outline-hidden',
    // Text styles
    'text-left text-sm/5 text-zinc-950 dark:text-white forced-colors:text-[CanvasText]',
    // Focus
    'data-focus:bg-zinc-950/[0.06] data-focus:text-zinc-950 dark:data-focus:bg-white/[0.08] dark:data-focus:text-white',
    // Disabled state
    ...
    // Icons
    '*:data-[slot=icon]:col-start-1 *:data-[slot=icon]:row-start-1 *:data-[slot=icon]:mr-2 *:data-[slot=icon]:-ml-0.5 *:data-[slot=icon]:size-4',
    ...
    // Avatar
    '*:data-[slot=avatar]:mr-2 *:data-[slot=avatar]:-ml-0.5 *:data-[slot=avatar]:size-5',
  )
...
export function DropdownDescription({
  ...
  return (
    <Headless.Description
      ...
      className={clsx(
        className,
        'col-span-2 col-start-2 row-start-2 text-xs/5 text-zinc-500 dark:text-zinc-400 forced-colors:group-data-focus:text-[HighlightText]',
      )}
    />
  )
}
    ...
    >
      {(Array.isArray(keys) ? keys : keys.split('')).map((char, index) => (
        <kbd
          key={index}
          className={clsx([
            'min-w-[2ch] text-center font-sans text-zinc-400 capitalize forced-colors:group-data-focus:text-[HighlightText]',
