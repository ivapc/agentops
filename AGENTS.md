...
- `/traces` has two tabs: Traces (end-to-end runs; utility traces filtered out) and Spans (`?tab=spans`, lazy-fetched) listing utility purpose-attr spans + sub-agent invocations (`invoke_agent` under `execute_tool`). Transitional — once the consumer-side normaliser ships (`docs/plans/agentops-conventions.md`), sub-agents become first-class trace rows keyed by `gen_ai.task.id` and the Spans tab shrinks to utility-only.
