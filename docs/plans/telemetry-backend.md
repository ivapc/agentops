...
- feeds `classifySpan(name, attrs)` normalization in agentops via a new `TelemetryProvider`
- hosts on **Azure**
...
- No local mirror in agentops itself; reads stay live via providers in `src/lib/telemetry/`.
...
- **Pros**: cheapest columnar option; portable off Azure; B-series burst credits fit agentops's spiky query load; mature exporter (ClickHouse Inc maintained); `TTL ... DELETE` handles retention declaratively
...
- **Cost**: $15–25/mo
- **Pros**: cheapest viable option; fully managed (backups, patches); familiar tooling; provider interface lets you swap to CH later without touching agentops
...
- **Stack**: keep AI ingest; bypass AI query API by cross-cluster-querying AI's underlying ADX from agentops
...
- Need HA? Single-VM CH has none. Acceptable for agentops?
