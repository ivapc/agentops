...
**Spans tab.** `InspectLayout` (`src/features/inspect/components/`): span tree + span detail (`DetailPanel`), with a Context tab whose token breakdown (`System prompts · Tool definitions · Messages · Prompt cache · Total`) is computed by `useBreakdowns` (`src/features/inspect/components/use-breakdowns.ts`) on top of `breakdownChat` in `src/features/inspect/logic/tokens.ts`. Hides naked `http` transport spans while keeping subtree rollups contiguous. The same components render the slide-over `InspectDrawer` used by both the sessions and traces lists.
...
| Support a new tokenizer family                                 | Extend the encoder resolution in `src/features/inspect/logic/tokens.ts` (currently Anthropic vs o200k-estimate). Lazy-load the encoder data                                                                                                               |
