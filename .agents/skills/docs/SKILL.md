...
- `topic` — what to record or update. Can be:
  ...
  - **a reference to current changes** ("what I just changed", "this branch", "the diff", "uncommitted work") → run `git diff` / `git log main..HEAD` to gather the actual content before placing or writing
...
2. **Locate or place.**
   ...
   - *Outside `explanation/`* (guides, reference, plans, top level): no numbering. Just kebab-case filenames; reading order lives in the folder `README.md`. `plans/` holds forward-looking proposals for unbuilt features. The glossary lives at `docs/reference/glossary.md`. (`decisions/` for ADRs isn't in the tree yet — create it on first need, three-digit ADR padding `004-…md`; don't scaffold empty.)
...
4. **Index follow-ups.**
   ...
   - Introduced a domain term → add a one-line entry to `docs/reference/glossary.md` linking to the new doc.
