...
| `purpose` field on a span                                           | If a span shows `"title_generation"` but the agentops env expects `gen_ai.operation.purpose`, check `CUSTOM_LLM_PURPOSE_FIELD`.                                                                                                                                                                                                                                                                                                                  |
| `key_drift.purpose_on_ancestor_not_on_chat`                         | Purpose tag lives on a parent custom Activity but not on the `chat` LLM span itself. agentops' `propagateInheritedAttrs` (`src/lib/spans.ts`) can lift `operationName` down, but only if the purpose key is recognized first. If the UI's spans view shows no tag on the chat row, either (a) producer should stamp purpose directly on the chat span, or (b) the producer's purpose key isn't in `conventions.ts` / `CUSTOM_LLM_PURPOSE_FIELD`. |
